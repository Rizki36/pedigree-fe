			...
			>
				{menus.map((menu) => (
					<Link
						...
						to={menu.path}
						className={cn(
							"flex items-center gap-x-2 [&.active]:text-neutral-50 [&.active]:font-semibold [&.active]:bg-teal-500 py-2 rounded-full",
							{
								"px-4": !collapse,
	...
	return (
		<DropdownMenu>
			<DropdownMenuTrigger className="py-2 border px-3 flex items-center rounded-lg">
				...
				/>
				<IoChevronDown className="text-neutral-500 ml-2 size-3" />
			</DropdownMenuTrigger>
			<DropdownMenuContent>
